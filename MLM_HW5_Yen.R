...
newdata_wf <- data.frame(satot = seq (0,30,30/25),ageint= seq(0,25,1),nonwhite = "White",sex97="Female")
newdata_wm <- data.frame(satot = seq (0,30,30/25),ageint= seq(0,25,1),nonwhite = "White",sex97="Male")
newdata_nwf <- data.frame (satot = seq(0,30,30/25),ageint= seq(0,25,1),nonwhite = "Non-white",sex97="Female")
newdata_nwm <- data.frame (satot = seq (0,30,30/25),ageint= seq(0,25,1), nonwhite = "Non-white",sex97="Male")

y_wf <-predict(smod5a,newdata_wf,re.form=NA)
y_wm <-predict(smod5a,newdata_wm,re.form=NA)
y_nwf<-predict(smod5a,newdata_nwf,re.form=NA)
y_nwm<-predict(smod5a,newdata_nwm,re.form=NA)

plot (newdata_wf$satot,y_wf,type="l",col="blue",lty=1,
      xlim= c(12,25),ylim= c(0,30),
      main="Increase substance over time",xlab="Age",
      ylab=" Predicted number of substance")
points (newdata_wm$satot,y_wm,type="l",lty=1,col="brown1")
points (newdata_nwf$satot,y_nwf,type="l",lty=1,col="olivedrab")
points (newdata_nwm$satot,y_nwm,type="l",lty=1,col="cyan")
legend(c(12,16),c(20,30),c("White Female","White Male","Non-white Female","Non-white Male"),col=c("blue","brown1","olivedrab","cyan"),lty=c(1,1,1,1))
